...
The recommended way to serialize a structure as JSON is to apply
  @ref{struct:to-json#1,,@code{struct:to-json}}.
-->
...
<function name="struct:to-json" as="xs:string">
  ...
  <variable name="result" as="xs:string*">
    <apply-templates mode="struct:to-json"
                    select="$struct" />
  </variable>
...
-->
<template mode="struct:to-json" priority="5"
          match="struct:array">
  <text>[</text>
    <apply-templates mode="struct:to-json"
                     select="node()" />
...
<!--
  ...
  Until items are typed, we have no choice but to serialize all items
    as strings.
-->
<template mode="struct:to-json" priority="4"
          match="struct:item">
...
<template mode="struct:to-json" priority="5"
          match="struct:dict">
  <text>{</text>
    <apply-templates mode="struct:to-json-dict"
                     select="node()" />
  <text>}</text>
</template>
...
<!--
  ...
  Until items are typed, we have no choice but to serialize all items
    as strings.
-->
<template mode="struct:to-json-dict" priority="5"
          match="struct:item[ @key ]">
  <sequence select="concat( '&quot;',
                            _struct:json-escape-str( @key ),
                            '&quot;:' )" />
...
<!--
  Error on unrecognized structures during dictionary JSON
    transformation.
-->
<template mode="struct:to-json-dict" priority="1"
          match="node()">
...
-->
<template mode="struct:to-json" priority="2"
          match="comment()">
</template>
...
-->
<template mode="struct:to-json" priority="1"
          match="node()">
  <sequence select="error( $struct:error-qname,
